...
/**
 * Main Class for CassandraJoins Java API.
 * Call first (once) CassandraJoins.connect()  static method  to get a new Cassandra Cluster connection (ClusterConn)
 * Then you can call (multiple times) CassandraJoins.join(ClusterConn, ...) static method  to join two Cassandra tables
 * To close the connection call  ClusterConn.close()
...
/** 
 *  Call CassandraJoins.connect() to get a new Cassandra Cluster connection
 *  @param node The IP address or the host name of the cassandra node to connect, eg: localhost
 *  @return A com.datastax.driver.core.Cluster object 
 */
...
/* If tableName doesn't exist on KeySpace returns false
 * If columnName doesn't exist on tableName returns false
 * If columnName exists on tableName returns true
 */
...
/* If columnName of tableName of KeySpace has an index-name(planas change) returns true
*/
...
/* If columnName of tableName of KeySpace is the only column of the PARTITION KEY of the table, returns true
*/
...
/* Checks if all PARTITION KEYS of  tableName are restricted in the whereRelations and the columnName, so a CQL select is allowed to be performed on the table*/ 
...
/* If columnName of tableName of KeySpace is a clustering key,  returns true
*/
...
/* Checks if all CLUSTERING KEYS preceeding  of columName in  tableName's clustering order are restricted in the whereRelations and the columnName, so a CQL select is allowed to be performed on the table*/ 
...
/* Combines the column names on the array with commas and creates the String for the Select clause to be used on a CQL query*/
...
/* Combines the WHERE clauses   on the array with PARENTHESIS and AND and creates the String for the WHERE clause to be used on  a CQL query*/
...
/* Returns the data type of a specific table column as String*/
...
/* If tableName exists in KeySpace returns true */
...
/* Returns true when the parameter is set , map or list*/
...
/*Creates a new empty table in KeySpace for the join results to be stores with name resultTableName and the selected columns of tableName1 and tableName2 without duplicating joining column*/
...
/* Performs a FULL JOIN of two tables using a LEFT and a RIGHT join and sortmergejoin algorithm*/ 
...
/* Performs a LEFT or a RIGHT JOIN of two tables using sortmergejoin algorithm*/ 
...
/* Returns as String the value of a specific column in a specific row  of tableName*/
private static String getVal(Cluster ClusterConn, String KeySpace, String tableName, String columnName, Row row) throws CassandraJoinsException
{
     ...
     case "timestamp":
           ...
           return new String(Long.toString(valdate.getTime())); //change by planas . it was Date not LocalDate and it was valdate.gettime()
     case "counter":
...
/*  JOIN  two tables using INDEX JOIN algorithm*/ 
...
/* Returns an array with all the column names of tableName*/
...
/* Checks an array of where clauses if they are syntactically correct on CQL*/
...
/**
 * Joins two tables  and creates a new table with the result columns
 * @throws CassandraJoinsException If parameters are wrong, tables or columns doesn't exists or join can't be performed
 * @param ClusterConn Connection to  Cassandra Cluster
 * @param KeySpace Cassandra Keyspace containing the tables for join
 * @param tableName1 Name of the first table to join
 * @param columnName1 Name of the column of the first table to be used for join
 * @param selectColumns1 Array containing the names of the columns of the first table we can to include on the join result
 * @param whereRelations1 Array containing all the CQL "where" clauses for the selections to be performed on the first table, each entry must has the format  <i>COLUMNNAME OP VALUE </i> where <i> COLUMNNAME  </i> is a column of the first table, <i> OP </i> is a valid operator in cql and <i>VALUE </i> is a value of a valid cql data type (including collection data types)  
 * @param tableName2 Name of the second table to join
 * @param columnName2 Name of the column of the second table to be used for join
 * @param selectColumns2 Array containing the names of the columns of the second table we can to include on the join result
 * @param whereRelations2 Array cointaining where clauses for the selections to be performed on the second table
 * @param rowsLimit The number of rows we want to use from each  table to perform  the join
 * @param resultTableName The name of the new table to be created containing the  result of JOIN operation
 * @param operator Must be "=" for traditional join or "CONTAINS" to perform  joins where ColumnName2 data type is a set, map or list containing values matching ColumnName1 
 */
...
if (!isCollectionType(ctype2))
           throw new  CassandraJoinsException("Joins using operator CONTAINS require the joining column of the second table to be on of types:set, map, list");
}
